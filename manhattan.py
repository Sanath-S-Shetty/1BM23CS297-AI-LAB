...
def astar_manhattan(start_state, goal_state):
    """Solves the 8-puzzle using A* with the Manhattan distance heuristic."""
    open_set = []
    heapq.heappush(open_set, (manhattan_distance(start_state, goal_state), 0, start_state, [])) # (f_cost, g_cost, state, path)
    visited = set()
    ...
    while open_set:
        ...
        for neighbor in get_neighbors(current_state):
            if neighbor not in visited:
                ...
                new_g_cost = g_cost + 1
                new_f_cost = new_g_cost + manhattan_distance(neighbor, goal_state)
...
astar_manhattan(start_state, goal_state)